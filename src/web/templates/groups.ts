...
export function getGroupsHTML(groups: any[]): string {
  ...
  if (monitoredGroups.length === 0) {
    return '<p class="text-gray-500">No hay grupos monitoreados. Usa el CLI para agregar grupos.</p>';
  }
    ...
    <ul class="space-y-2">
      ${monitoredGroups.map(group => `
        <li class="flex items-center justify-between p-2 bg-gray-50 rounded">
          <span>${group.name} ${group.participants ? `(${group.participants} participantes)` : ''}</span>
          <button 
